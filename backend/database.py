...
class Database:
    ...
    def add_user(self, id, name, role, face_embedding, voice_embedding):
        if hasattr(face_embedding, "detach"):
            face_bytes = face_embedding.detach().cpu().numpy().astype(np.float32).tobytes()
        else:
            face_bytes = np.asarray(face_embedding, dtype=np.float32).tobytes()

        if hasattr(voice_embedding, "detach"):
            voice_bytes = voice_embedding.detach().cpu().numpy().astype(np.float32).tobytes()
        else:
            voice_bytes = np.asarray(voice_embedding, dtype=np.float32).tobytes()
    ...
    def get_embedding(self, user):
        face_embedding = np.frombuffer(user.face_embedding, dtype=np.float32).reshape(1, 128)
        voice_embedding = np.frombuffer(user.voice_embedding, dtype=np.float32).reshape(1, 128)
